async function renderArticle() {
  ...
  if (!post) {
    root.innerHTML = '<div class="empty-state">게시글을 찾을 수 없습니다. <a href="' + SITE_BASE + 'index.html">홈으로 돌아가기</a></div>';
    return;
  }

  document.title = post.title + ' — 그라운드';

  const catUrl = SITE_BASE + 'pages/category.html?cat=' + post.category;
  const paragraphs = post.content.map((p) => '<p>' + p + '</p>').join('');
  const durationText = post.video ? ' · 영상 ' + post.duration : '';

  root.innerHTML =
    '<div class="breadcrumb"><a href="' + SITE_BASE + 'index.html">홈</a> / <a href="' + catUrl + '">' + post.categoryLabel + '</a></div>' +
